<?php

namespace Prettus\Repository\Tests\Commands;

use Illuminate\Support\Facades\File;
use Prettus\Repository\Tests\TestCase;
use Prettus\Repository\Tests\Concerns\CommandTestTrait;

/**
 * Class ValidatorCommandTest
 *
 * @package Prettus\Repository\Tests\Command
 * @author Anitche Chisom <anitchec.dev@gmail.com>
 */
class ValidatorCommandTest extends TestCase
{
    use CommandTestTrait;

    protected function setUp(): void
    {
        parent::setUp();

        $this->prepValidatorClass();
    }

    /** @test */
    public function it_creates_a_new_presenter_class()
    {
        $this->assertFalse(File::exists($this->fooValidatorClass));

        // Run the make command
        $this->artisan('make:validator Foo');

        // Assert a new file is created
        $this->assertTrue(File::exists($this->fooValidatorClass));

        // Assert file content is as expected
        $this->assertEquals(file_get_contents($this->validatorStub()), file_get_contents($this->fooValidatorClass));
    }
}
